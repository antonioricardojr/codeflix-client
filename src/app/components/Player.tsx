  ...
  return (
    <div className='flex h-screen justify-center align-middle'>
        <MediaPlayer 
            ...
            >
            <Link href="/">
                <ArrowLeftIcon className='w-6 h-6 media-playing:opacity-0 absolute left-8 top-8 z-50  transition-opacity duration-300 ease-in-out hover:opacity-70' />
            </Link>
            ...
            <MediaProvider className="relative mx-auto flex aspect-video max-w-fit justify-center rounded-md align middle">
                <div className="media-playing:opacity-0 invisible absolute left-8 top-96 z-50 transition-opacity duration-500 md:visible">
                    <h1 className="text-2xl font-bold md:text-4xl lg:text-7xl">{movie.title}</h1>
                    <p className="text-shadow-md mt-4 max-w-xs text-xs md:max-w-lg md:text-lg lg:max-w-2xl">{movie.description}</p>
                </div>
            </MediaProvider>
        </MediaPlayer>        
    </div>
